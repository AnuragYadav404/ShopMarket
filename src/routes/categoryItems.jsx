...
function Card({title, image, rating}) {
    return (
        <div className="cards">
            <div className="cardsImg"><img src={image} alt={title} loading="lazy"/></div>
            <h4>{title}</h4>
                    ...
                    {items.map((item) => {
                        return (
                            <li key={item.id}>
                                ...
                                <Card title={item.title} image={item.image} rating={item.rating} id={item.id}/>
                                <Link to={`/shop/item/${item.id}`}><button>Buy Now!</button></Link>
                            </li>
                        )
                    })}
                </ul>
